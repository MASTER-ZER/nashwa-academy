              ...
              <form onSubmit={handleSaveEditProfile} className="space-y-4 text-xs">
                ...
                <div className="space-y-1">
                  <label className="font-bold text-slate-700 dark:text-slate-300 flex items-center justify-between text-xs">
                    <span>المجموعة الدراسية ومواعيد الحصص:</span>
                    <span className="text-[10px] text-amber-600 dark:text-amber-400 font-bold">
                      (يتطلب موافقة المس لتغيير الميعاد)
                    </span>
                  </label>
                  <select
                    value={editFormData.groupId}
                    onChange={(e) => setEditFormData({ ...editFormData, groupId: e.target.value })}
                    className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white font-bold focus:outline-none focus:border-brand-500 text-xs"
                  >
                    {groups.map((g) => (
