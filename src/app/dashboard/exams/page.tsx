...
import { Award, Plus, MessageSquare, Phone, CheckCircle2, Search, Sparkles, Send } from 'lucide-react';
...
export default function ExamsDashboardPage() {
  ...
  // Scores input state (local temporary state before saving)
    ...
    <div className="space-y-6 py-2">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <label className="text-xs font-bold text-slate-600 whitespace-nowrap">اختر الامتحان:</label>
          <select
            value={selectedExamId}
            onChange={(e) => setSelectedExamId(e.target.value)}
            className="w-full sm:w-96 px-3 py-2 text-xs font-bold rounded-xl border border-slate-300 bg-slate-50 focus:border-brand-500 focus:outline-none"
          >
            {data.exams.map((ex) => (
        ...
        {selectedExam && (
          <div className="text-xs text-slate-500 font-semibold">
            الدرجة النهائية: <strong className="text-brand-600 font-bold">{selectedExam.totalScore} درجة</strong>
          </div>
        )}
      </div>
              ...
              {activeStudents.map((std) => {
                ...
                return (
                  <tr key={std.id} className="hover:bg-slate-50/80 transition">
                    ...
                    <td className="p-4 text-slate-500 text-[11px]">{grp ? grp.name : '—'}</td>
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-1.5">
                        <input
                          ...
                          className="w-16 px-2 py-1.5 text-center font-black text-sm rounded-lg border border-slate-300 focus:border-brand-500 focus:outline-none"
                        />
                    ...
                    <td className="p-4">
                      <input
                        ...
                        className="w-full sm:w-48 px-2.5 py-1.5 text-xs rounded-lg border border-slate-300 focus:border-brand-500 focus:outline-none"
                      />
                    </td>
      ...
      {isAddExamOpen && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-4">
            <h2 className="text-lg font-bold text-slate-900">إضافة امتحان ورقي جديد</h2>

            <form onSubmit={handleCreateExam} className="space-y-3 text-xs">
              <div className="space-y-1">
                <label className="font-bold text-slate-700">عنوان الامتحان</label>
                <input
                  type="text"
                  placeholder="مثال: اختبار الباب الأول - الكيمياء ومركز العلوم"
                  value={newExamTitle}
                  onChange={(e) => setNewExamTitle(e.target.value)}
                  className="w-full px-3 py-2 rounded-xl border border-slate-300 focus:outline-none focus:border-brand-500 font-bold"
                  required
                />
              </div>

              <div className="space-y-1">
                <label className="font-bold text-slate-700">الدرجة النهائية</label>
                <input
                  type="number"
                  min={5}
                  max={100}
              ...
              <div className="space-y-1">
                <label className="font-bold text-slate-700">تاريخ الامتحان</label>
                <input
                  type="date"
                  value={newExamDate}
