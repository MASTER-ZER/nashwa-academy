...
export async function generateStudentReportCardCanvas(params: {
  ...
  const customComment = customNote?.trim() || (
    examResults.length === 0
      ? `نرحب بالطالب (${student.name}) ونتمنى له بداية موفقة في منهج العلوم المتكاملة 🌸. لم تُعقد اختبارات شهرية حتى الآن، ونحثه على الالتزام بحضور الحصص والتفوق في الاختبارات القادمة.`
      : avgPercent >= 85
      ? `نبارك لولي الأمر على تفوق الطالب (${student.name}) والتزامه المتميز في الحصص والامتحانات، ونتمنى له دوام النجاح والتألق.`
