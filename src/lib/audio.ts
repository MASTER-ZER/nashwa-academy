// Web Audio API generator for instant zero-latency feedback
...
class SoundEngine {
  ...
  // Warning Sound (Unpaid subscription / Action needed)
  playWarningAlert() {
    try {
      const ctx = this.getAudioContext();
      if (!ctx) return;
